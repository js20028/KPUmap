...
class StoreActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //지도에 상세페이지의 가게 표시
        detail_mapButton.setOnClickListener{
            var title2: TextView = findViewById(R.id.title_detail)
            var stName = title2.text.toString()
            val num : Int = 1

            val bun = Bundle()
            bun.putSerializable("imageArr", storeImageArray)
            bun.putSerializable("num",num)
            bun.putSerializable("stName",stName)
            val intent = Intent(this, KpuGpsActivity::class.java)
